...
const ImageSlider = ({ images }) => {
	...
	return (
		<Box backgroundImage={`url(${slides[currentIndex].src})`} backgroundSize="cover" backgroundPosition="center" height="650px" maxWidth="1440px" margin="auto" py={8} px={4} rounded="2xl" position="relative">
			<Flex position="absolute" top="50%" left="5" transform="translateY(-50%)" alignItems="center">
				<IconButton aria-label="Previous Slide" icon={<ArrowLeftIcon />} size="lg" onClick={prevSlide} mr={2} />
			</Flex>
			<Flex position="absolute" top="50%" right="5" transform="translateY(-50%)" alignItems="center">
				<IconButton aria-label="Next Slide" icon={<ArrowRightIcon />} size="lg" onClick={nextSlide} ml={2} />
			</Flex>
		</Box>
	);
};
