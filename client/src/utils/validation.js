...
export const signupValidationSchema = [
	yup.object().shape({
		...
		phoneNumber: yup.string().matches(/^\d+$/, "Phone Number must only contain digits").min(9, "Phone Number must be at least 10 digits").required("Phone Number is required"),
	}),
	yup.object().shape({
		username: yup
			.string()
...
export const loginValidationSchema = yup.object().shape({
	username: yup.string().min(3, "Invalid Email or Username"),
